    ...
    CASE
      WHEN h.data->'error_data'->>'message' IS NOT NULL THEN
        ...
        ELSE
          ...
          END
        END
      WHEN h.execution_finish IS NULL AND h.pid IS NULL THEN
...
-- LEFT JOIN
--     pg_catalog.pg_stat_activity s ON (
--         s.pid = h.pid
--         -- AND s.backend_type = 'TimescaleDB Background Worker Scheduler'
--         AND s.backend_start <= h.execution_start
--         AND s.state = 'active'
--         AND s.query ~ j.proc_name
--     )
WHERE
